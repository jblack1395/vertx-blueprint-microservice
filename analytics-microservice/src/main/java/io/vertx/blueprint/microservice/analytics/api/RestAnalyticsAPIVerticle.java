package io.vertx.blueprint.microservice.analytics.api;
 ...
 * @author Eric Zhao
 */
public class RestAnalyticsAPIVerticle extends RestAPIVerticle {

  public static final String SERVICE_NAME = "analytics-rest-api";
  ...
  public RestAnalyticsAPIVerticle(AnalyticsService service) {
  ...
  private void apiAdd(RoutingContext context) {
    try {
      Analytics product = new Analytics(new JsonObject(context.getBodyAsString()));
      service.addAnalytics(product, resultHandler(context, r -> {
        String result = new JsonObject().put("message", "product_added")
          .put("productId", product.getAnalyticsId())
          .encodePrettily();
  ...
  private void apiRetrieve(RoutingContext context) {
    ...
    service.retrieveAnalytics(productId, resultHandlerNonEmpty(context));
  }

  private void apiRetrievePrice(RoutingContext context) {
    String productId = context.request().getParam("productId");
    service.retrieveAnalyticsPrice(productId, resultHandlerNonEmpty(context));
  }
  ...
  private void apiRetrieveByPage(RoutingContext context) {
    try {
      ...
      service.retrieveAnalyticssByPage(page, resultHandler(context, Json::encodePrettily));
    } catch (Exception ex) {
  ...
  private void apiRetrieveAll(RoutingContext context) {
    service.retrieveAllAnalyticss(resultHandler(context, Json::encodePrettily));
  }
  ...
  private void apiDelete(RoutingContext context) {
    String productId = context.request().getParam("productId");
    service.deleteAnalytics(productId, deleteResultHandler(context));
  }

  private void apiDeleteAll(RoutingContext context, JsonObject principle) {
    service.deleteAllAnalyticss(deleteResultHandler(context));
  }
